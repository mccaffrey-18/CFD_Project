...
% Top lid (j=jmax): u=U_lid, v=0
j = jmax;
for i = 1:imax
    u(i,j,2) = uinf;   % u = lid speed
    u(i,j,3) = zero;   % v = 0
end
...
%************************************************************************
function [dt_max] = compute_time_step(dt_max)
%
...
for j=2:jmax-1
    for i=2:imax-1
        ...
        lambda_x = half*(abs(u_local)+sqrt(u_local^2+four*beta2));
        lambda_y = half*(abs(v_local)+sqrt(v_local^2+four*beta2));
        lambda_max = max(lambda_x, lambda_y);

        dtconv = min(dx,dy)/lambda_max;

        dt(i,j) = cfl*min(dtvisc, dtconv);
        
        dt_max = min(dt(i,j), dt_max);
    end
end
end
%************************************************************************
...
for j=2:jmax-1
    for i=2:imax-1
        ...
        u(i,j,1) = uold(i,j,1) + dt(i,j)*beta2*(artviscx(i,j)+artviscy(i,j)-rho*dudx-rho*dvdy);
        u(i,j,2) = uold(i,j,2) + dt(i,j)/rho*(-rho*uold(i,j,2)*dudx-rho*uold(i,j,3)*dudy-dpdx+rmu*d2udx2+rmu*d2udy2);
        u(i,j,3) = uold(i,j,3) + dt(i,j)/rho*(-rho*uold(i,j,2)*dvdx-rho*uold(i,j,3)*dvdy-dpdy+rmu*d2vdx2+rmu*d2vdy2);
    ...
    for k = 1:neq
        ...
        for j = 2:jmax-1
            ...
            for i = 2:imax-1
                ...
                % Accumulate norms
                DEsum = DEsum + DE;
                DEsumsq = DEsumsq + DE^2;
                DEmax = max(DEmax, DE);
            end
        end

        % Normalize L1 and L2
        rL1norm(k) = DEsum / (Nx*Ny);
        rL2norm(k) = sqrt(DEsumsq / (Nx*Ny));
        rLinfnorm(k) = DEmax;
    end
